...
export default function ProjectDashboard({ initialProjects, initialCsrfToken }: Props) {
  ...
  return <main className="pc-dashboard">
    ...
    {projects.length === 0 ? <section className="pc-dashboard-empty"><h3>No projects yet</h3><p>Create a blank canvas or start from the editable Cantor-set sample.</p></section> : <section className="pc-project-grid" aria-label="Projects">
      {projects.map((project) => <article key={project.id} className="pc-project-card" data-project-id={project.id}>
        <a className="pc-project-thumbnail" href={`/projects/${project.id}`} aria-label={`Open ${project.title}`}>{project.thumbnail ? <span data-thumbnail-sha={project.thumbnail.sha256}>{project.thumbnail.mimeType.replace("image/", "").toUpperCase()} thumbnail</span> : <><span aria-hidden="true">∴</span><small>No thumbnail yet</small></>}</a>
        <div className="pc-project-card-body"><label><span>Title</span><input value={titles[project.id] ?? project.title} maxLength={160} onChange={(event) => setTitles((current) => ({ ...current, [project.id]: event.target.value }))} /></label>
          ...
          <div className="pc-project-actions"><a href={`/projects/${project.id}`}>Open</a><button type="button" onClick={() => void rename(project)} disabled={Boolean(pending) || titles[project.id]?.trim() === project.title}>Save title</button><button type="button" onClick={() => void duplicate(project)} disabled={Boolean(pending)}>Duplicate</button><button className="pc-danger-button" type="button" onClick={() => void remove(project)} disabled={Boolean(pending)}>Delete…</button></div>
        </div>
      </article>)}
    </section>}
  </main>;
